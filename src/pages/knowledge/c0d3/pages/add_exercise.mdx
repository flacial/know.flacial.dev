...
For the inputs, we chose to use the `FormCard` component from the components library. This component takes an array of objects. Each object represents the input `type`, `value`, and `title`. The input could be a markdown input or a regular input.
...
One solution we thought of is to abuse the `formChange` helper and run it for each input before adding the exercise. This method will validate each input and set its error message if it's invalid.

THe solution works, but it's not ideal. The component `FormCard` must be modified to validate each input before submitting. Therefore, we chose to add this feature to `FormCard` first.
