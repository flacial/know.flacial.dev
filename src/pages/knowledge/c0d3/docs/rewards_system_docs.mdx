...
- After a student complete JS4, a server should be created for them which they'll use to practice the content of JS5 in it.
...
After the student complete JS4, The student will input their SSH key into a page under `/[username]/server`, this SSH will be used in the next action item to ensure that the student will have access to the server (and the admins).
...
2. The server will exist as long as they use it
   - "using it" means resources. They might have a server running which will use some of the resournnces. Monitoring their usage based on this metric won't work
   - A better way is to monitor the most recent times they SSH-ed into the server. If the last time they accessed it is x days old, remove the server. To achieve this, we must create a custom image that'll have the system to monitor SSH access.
